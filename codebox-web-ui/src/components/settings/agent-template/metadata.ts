/**
 * Static metadata for the agent-template form UI.
 *
 * ``ALLOWED_FIELDS`` and ``OPS_BY_TYPE`` mirror
 * ``codebox-orchestrator/src/codebox_orchestrator/agent_template/application/allowed_fields.py``.
...
export function triggerKindMeta(kind: AgentTemplateTriggerKind): TriggerKindMeta {
  return TRIGGER_KINDS.find((t) => t.value === kind) ?? TRIGGER_KINDS[0]
}

export function triggerIcon(kind: AgentTemplateTriggerKind): HugeIcon {
...
export const OPS_BY_TYPE: Record<FieldType, Array<AgentTemplateFilterOp>> = {
...
export const OP_LABELS: Record<AgentTemplateFilterOp, string> = {
  eq: "equals",
  in: "is one of",
...
export const OP_HINTS: Record<AgentTemplateFilterOp, string> = {
...
export interface WorkspaceModeMeta {
  ...
  title: string
  description: string
  availableWhen: (kind: AgentTemplateTriggerKind) => boolean
}
...
export const WORKSPACE_MODES: ReadonlyArray<WorkspaceModeMeta> = [
  ...
  },
  {
    ...
    title: "Pinned branch",
    description:
      "Always work on a fixed repository + branch. Required for scheduled templates.",
    availableWhen: () => true,
  },
] as const

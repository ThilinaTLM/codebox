...
interface AgentTemplateDryRunPanelProps {
  projectSlug: string
...
export function AgentTemplateDryRunPanel({
  projectSlug,
  template,
}: AgentTemplateDryRunPanelProps) {
  const isScheduled = template.trigger_kind === "schedule"
  const trigger = triggerKindMeta(template.trigger_kind)
  const scenarios = useMemo(
    () => scenariosFor(template.trigger_kind),
          ...
          <>
            ...
            <FormField
              ...
              description={
                jsonError
                  ? undefined
                  : "Sent to the template engine verbatim. Only fields used by filters + variables are needed."
              }
              action={
                <Button
                  type="button"
...
function DryRunResultView({ result }: { result: AgentTemplateDryRunResult }) {
  return (
    <div className="space-y-4">

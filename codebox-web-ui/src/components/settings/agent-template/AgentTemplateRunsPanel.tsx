...
import { useInfiniteAgentTemplateRuns } from "@/net/query"
...
interface AgentTemplateRunsPanelProps {
  projectSlug: string
  templateId: string
}

export function AgentTemplateRunsPanel({
  projectSlug,
  templateId,
}: AgentTemplateRunsPanelProps) {
  return (
    <RunsPanelInner
...
function RunsPanelInner({
  ...
  const query = useInfiniteAgentTemplateRuns(projectSlug, templateId, {
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Recent triggers for this template.
          </p>
        </div>
        <Button
          type="button"

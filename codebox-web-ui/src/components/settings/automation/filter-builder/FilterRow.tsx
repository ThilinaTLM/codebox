...
interface FilterRowProps {
  ...
  predicate: AutomationFilterPredicate
  error?: string
  onUpdate: (patch: Partial<AutomationFilterPredicate>) => void
  onRemove: () => void
  disabled?: boolean
}
...
export function FilterRow({
  ...
  const handleFieldChange = (newField: string) => {
    ...
    const newOp: AutomationFilterOp = newAvailableOps.includes(
      predicate.op
    )
      ? predicate.op
      : newAvailableOps[0]
  ...
  const handleOpChange = (newOp: AutomationFilterOp) => {
    const listy =
    ...
    >
      <div className="grid gap-2 sm:grid-cols-[minmax(160px,1fr)_minmax(140px,1fr)_minmax(200px,2fr)_auto]">
        ...
        <div className="space-y-1">
          <Select
            value={predicate.op}
            onValueChange={(v) => v && handleOpChange(v as AutomationFilterOp)}
            disabled={disabled}
          >

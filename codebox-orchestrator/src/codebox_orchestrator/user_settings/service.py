"""Business logic for per-user settings — encryption, masking, GitHub config."""
...
class UserSettingsService:
    ...
    async def get_settings(self, user_id: str) -> UserSettingsView | None:
        settings = await self._repo.get(user_id)
        if settings is None:
            return None
        return self._to_view(settings)

    async def get_raw(self, user_id: str) -> UserSettings | None:
        """Return the raw ORM object (for internal use — encrypted fields intact)."""
        return await self._repo.get(user_id)

    async def get_default_profile_id(self, user_id: str) -> str | None:
        settings = await self._repo.get(user_id)
        return settings.default_llm_profile_id if settings else None

    async def get_tavily_api_key(self, user_id: str) -> str | None:
    ...
    def github_configured(self, settings: UserSettings | None) -> bool:
        """Return True if the user has a complete GitHub App config."""
        if settings is None:
            return False
        return bool(
    ...
    async def update_settings(self, user_id: str, **kwargs: str | None) -> UserSettingsView:
        """Update user settings.  Sensitive fields are encrypted before storage."""
        ...
        settings = await self._repo.upsert(user_id, **fields)
        return self._to_view(settings)

    async def clear_default_if_matches(self, user_id: str, profile_id: str) -> None:
        """Clear the default LLM profile if it matches *profile_id*."""
        settings = await self._repo.get(user_id)
        if settings and settings.default_llm_profile_id == profile_id:
            await self._repo.upsert(user_id, default_llm_profile_id=None)

    # ── Helpers ─────────────────────────────────────────────

    def _to_view(self, s: UserSettings) -> UserSettingsView:

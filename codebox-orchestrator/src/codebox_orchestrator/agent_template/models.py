"""SQLAlchemy ORM models for agent templates, runs and the scheduler lock."""
...
class AgentTemplate(Base):
    """A project-scoped automation rule mapping a trigger to a box spawn."""

    __tablename__ = "agent_templates"
    __table_args__ = (
        UniqueConstraint("project_id", "name", name="uq_agent_templates_project_name"),
        Index("ix_agent_templates_project_enabled", "project_id", "enabled"),
        Index("ix_agent_templates_trigger_kind", "project_id", "trigger_kind"),
        Index("ix_agent_templates_next_run_at", "next_run_at"),
    )
...
class AgentTemplateRun(Base):
    """A record of a trigger firing against a template — 0..N boxes per event."""

    __tablename__ = "agent_template_runs"
    __table_args__ = (
        Index("ix_agent_template_runs_template", "template_id", "created_at"),
        Index("ix_agent_template_runs_event", "github_event_id"),
        Index("ix_agent_template_runs_project", "project_id", "created_at"),
    )
...
class SchedulerLock(Base):
    """Leader-election row for the agent-template scheduler across replicas."""

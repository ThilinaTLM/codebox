"""Business logic for automations: validation, cron parsing, scheduling hooks."""
...
MAX_SCHEDULED_AUTOMATIONS_PER_PROJECT = 100
MIN_SCHEDULE_INTERVAL_SECONDS = 5 * 60  # 5 minutes
AUTOMATION_NAME_RE = re.compile(r"^[\w\s][\w\s\-]*$")


class SchedulerHandle(Protocol):
    """Minimal interface the scheduler exposes back to the service layer."""

    def notify_automation_changed(self, automation: Automation) -> None: ...
...
class AutomationService:
    """Commands + queries for project-scoped automations."""
    ...
    async def list(
        ...
        *,
        ...
        automations = await self._repo.list(project_id, trigger_kind=trigger_kind, enabled=enabled)
        return [self._to_response(a) for a in automations]

    async def get(self, project_id: str, automation_id: str) -> AutomationResponse | None:
        automation = await self._repo.get(automation_id, project_id=project_id)
    ...
    async def create(
        ...
        try:
            automation = await self._repo.create(automation)
        except IntegrityError as exc:
            raise ValueError("Automation name must be unique within project") from exc
        if automation.trigger_kind == "schedule" and automation.enabled:
            self._scheduler_handle.notify_automation_changed(automation)
        return self._to_response(automation)
    ...
    async def update(
        ...
        existing = await self._repo.get(automation_id, project_id=project_id)
        if existing is None:
            return None
        ...
        try:
            updated = await self._repo.update(automation_id, project_id=project_id, **fields)
        except IntegrityError as exc:
            raise ValueError("Automation name must be unique within project") from exc
        if updated is None:
            return None
        if updated.trigger_kind == "schedule" and updated.enabled:
            self._scheduler_handle.notify_automation_changed(updated)
        return self._to_response(updated)

    async def delete(self, project_id: str, automation_id: str) -> bool:
        return await self._repo.delete(automation_id, project_id=project_id)
        ...
        *,
        ...
        if data.trigger_kind == "schedule":
            if not data.schedule_cron:
                raise ValueError("schedule_cron: required for scheduled automations")
    ...
    async def _enforce_scheduled_cap(self, project_id: str) -> None:
        ...
        if count >= MAX_SCHEDULED_AUTOMATIONS_PER_PROJECT:
            raise ValueError(
                f"At most {MAX_SCHEDULED_AUTOMATIONS_PER_PROJECT} "
                "scheduled automations per project"
            )
    ...
    @staticmethod
    def _to_response(automation: Automation) -> AutomationResponse:
        filters = None
        if automation.trigger_filters is not None:
            filters = [
                TriggerFilterPredicate.model_validate(p) for p in automation.trigger_filters
            ]
        return AutomationResponse(
            ...
            llm_profile_id=automation.llm_profile_id,
